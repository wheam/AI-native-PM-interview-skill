...
> **Agent 深度探针（agent 方向岗位高频，追到架构级）**：
> - "Agent 和 chatbot、和 workflow 的**本质区别**？"（chatbot=对话；workflow=人预设的固定编排；agent=模型自主规划+调用工具完成任务。能讲清"为什么这不是个 workflow"是关键）
> - "你怎么理解一个 agent 框架的 **harness**？"（harness=模型之外的编排/脚手架：planning、tool 调度、记忆、纠错兜底；好 harness 补模型短板，**模型变强时 harness 反而该变薄**）
> - "**computer-use / GUI agent** 的解决方案和核心难点？"（方案：截图/无障碍树感知→规划→点击输入闭环；难点：感知准确率、长任务可靠性、误操作不可逆、延迟与成本、跨步骤状态漂移）
> - "同一个 agent 能力，**部署在云端还是本地**怎么判断？"（本地=隐私/低延迟/能碰本地文件与系统；云端=算力/可常驻定时长跑/易协同；按数据敏感度、是否需常驻、延迟要求权衡）
> - "怎么设计 / 迭代一个 **agent skill**？"（怎么选场景与行业、怎么定优先级、怎么衡量好坏、用什么信号指导迭代方向）

> **AI 项目深挖追问链**（深挖你做过的 AI 项目时会一路顺着追，任一环断就露馅）：
> 怎么定**准召的指标与目标** → 怎么**调优** → 模型用什么**方式训练** → 怎么**选模型** → 怎么**评估**。
...
- **评判一个你熟悉的现有 AI 产品：哪里做得不好？给解法。**（考批判性 + 解决方案，不能只吐槽；要落到"为什么不好 + 怎么改 + 预期效果"）
- **为一个 agent 平台设计或迭代一类 agent skill**：怎么选场景与行业、怎么定优先级、怎么衡量好坏、怎么指导迭代方向？
...
5. **现场看 demo / 日常 AI 使用**：面试官当场让你打开**你做过的 demo**或**你平时怎么用 AI**，边演示边问。考的是"真用真做"还是"嘴上会"。常见可展示物：llm-wiki 类个人知识库、用 openclaw 等 agent 管理本地文件 / 设置云端定时任务、自动化取数 / 调研工具等。**备考：手头常备 2-3 个能立刻打开演示的真实 demo/工作流。**
